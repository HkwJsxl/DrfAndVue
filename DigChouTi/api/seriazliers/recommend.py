...
class RecommendTopicModelSerializer(serializers.ModelSerializer):
    class Meta:
        model = models.Topic
        fields = ('id', 'title', 'is_hot')


class RecommendNewsModelSerializer(serializers.ModelSerializer):
    """收藏"""

    zone = serializers.CharField(source='get_zone_display')
    topic = RecommendTopicModelSerializer()
...
class RecommendModelSerializer(serializers.ModelSerializer):
    news_info = RecommendNewsModelSerializer(read_only=True, source='news')
